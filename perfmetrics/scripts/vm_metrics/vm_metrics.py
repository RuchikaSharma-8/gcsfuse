...
PROJECT_NAME = 'projects/gcsfuse-interns-project-2023'
...
REQUEST_COUNT_NEW_READER = Metric(
    metric_type=REQUEST_COUNT_METRIC_TYPE,
    factor=1,
    aligner='ALIGN_DELTA',
    extra_filter='metric.labels.fs_op == "NewReader"',
    reducer='REDUCE_SUM',
    group_fields=['metric.labels'])

REQUEST_COUNT_CREATE_OBJECT = Metric(
    metric_type=REQUEST_COUNT_METRIC_TYPE,
    factor=1,
    aligner='ALIGN_DELTA',
    extra_filter='metric.labels.fs_op == "CreateObject"',
    reducer='REDUCE_SUM',
    group_fields=['metric.labels'])

REQUEST_COUNT_STAT_OBJECT = Metric(
    metric_type=REQUEST_COUNT_METRIC_TYPE,
    factor=1,
    aligner='ALIGN_DELTA',
    extra_filter='metric.labels.fs_op == "StatObject"',
    reducer='REDUCE_SUM',
    group_fields=['metric.labels'])
...
class VmMetrics:
  ...
  # def fetch_metrics_and_write_to_google_sheet(self, start_time_sec,
  #     end_time_sec, instance, period, test_type, worksheet_name):

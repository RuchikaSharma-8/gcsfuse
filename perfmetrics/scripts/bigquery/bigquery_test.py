...
Usage from bigquery directory:
  python3 bigquery_test.py
...
import constants
from experiments_gcsfuse_bq import ExperimentsGCSFuseBQ
...
class TestExperimentsGCSFuseBQ(unittest.TestCase):
  @patch('experiments_gcsfuse_bq.bigquery')
  ...
  def test_insert_rows_with_error(self):
    ...
    self.assertEqual(
        ...
        f'Error inserting data to BigQuery tables: {error_result}'
  ...
  def test_get_experiment_configuration_id_insert_new_configuration(self, mock_uuid4):
    ...
    result = self.experiments.get_experiment_configuration_id(
        GCSFUSE_FLAGS, BRANCH, END_DATE, CONFIG_NAME
    )
    ...
    self.client.insert_rows.assert_called_once_with(table_mock, [(
        config_id, CONFIG_NAME, GCSFUSE_FLAGS, BRANCH, END_DATE
  ...
  def test_get_experiment_configuration_id_update_existing_configuration(self):
    ...
    job_mock.__iter__.side_effect = lambda: iter([
        MagicMock(**{'get.return_value': VALID_CONFIG_ID})
    ])
    ...
    self.assertEqual(self.experiments._execute_query.call_count, 2)  # Check that in total 2 queries were executed

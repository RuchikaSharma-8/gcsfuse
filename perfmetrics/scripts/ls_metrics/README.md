...
7. Flag --gcsfuse_flags (required): GCSFUSE flags with which the list tests bucket will be mounted.
8. Flag --command (required): Takes as input a string, which is the command to run the tests on.
9. config_file (required): Path to the JSON config file which contains the details of the tests.
